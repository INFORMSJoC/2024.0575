...
class ASTRODF(Solver):
    ...
    def __init__(self, name="ASTRODF", fixed_factors=None):
        ...
        self.specifications = {
            ...
            "sampling_version": {
                "description": "0: ASTRO-DF, 1: ASTRO-DF-VM1 (lambda), 2: ASTRO-DF-VM2 (variance model), 3: ASTRO-DF-VM3 (hybrid)",
                "datatype": float,
                "default": 3
            },
            "cv": {
                "description": "C_v for Lipschitz Constant of the variance function",
                "datatype": float,
                "default": 10
            },
            "reguralized_objective": {
                "description": "reguarlized objective function with variance estimate?",
                "datatype": bool,
                "default": False
            },
    ...
    def construct_model(self, x_k, delta, k, problem, expended_budget, kappa, new_solution, visited_pts_list, num_implementation):
        ...
        reguralized_objective = self.factors["reguralized_objective"]
        ...
        while True:
            ...
            for i in range(2 * problem.dim + 1):
                ...
                if (k == 1) and (i == 0):
                    if reguralized_objective == False:
                ...
                # if iteration k-1 is unsuccessful, we can reuse the replications for x_k
                    ...
                    # Adaptive Sampling
                        ...
                        while True:
                            ...
                            expended_budget += 1 + overhead_costs
                            sample_size += 1
                            sig2 = new_solution.objectives_var
                ...
                # for new points, we need to run the simulation
                    ...
                    if sampling_version == 3:
                        ...
                        if k > 1 and ind_success == 1:
                            ...
                            if estimated_var > sig2_centerpoint + self.factors["cv"]*delta_k:
                        ...
                        pilot_run = int(max(lambda_min, .3 * problem.dim) - 3)
                        problem.simulate(design_set_solution, pilot_run)
                        expended_budget += pilot_run + overhead_costs
                        sample_size = pilot_run
                        ...
                        # adaptive sampling
                        while True:
                            problem.simulate(design_set_solution, 1)
                            expended_budget += 1 + overhead_costs
                            sample_size += 1
                            sig2 = design_set_solution.objectives_var
                            if sample_size >= self.get_stopping_time(k, sig2, delta_k, kappa, problem.dim) or expended_budget >= budget:
    ...
    def iterate(self, k, delta_k, delta_max, problem, visited_pts_list, new_x, expended_budget, budget_limit, recommended_solns, intermediate_budgets, kappa, new_solution, var_data, num_implementation):
        ...
        if norm(grad) != 0:
            ...
            if reguralized_objective == False:
                ...
                fval_tilde = -1 * problem.minmax[0] * candidate_solution.objectives_mean + pf_constant * candidate_solution.objectives_var
