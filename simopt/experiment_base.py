...
class ProblemSolver(object):
    ...
    def run(self, n_macroreps: int):
        ...
        with Pool(processes=7) as process_pool:
...
class ProblemSolverIT(object):
    ...
    def __init__(self, iteration_num: Union[int, None] = None, solver_name: Union[str, None] = None, problem_name: Union[str, None] = None, solver_rename: Union[str, None] = None, problem_rename: Union[str, None] = None, solver: Union["Solver", None] = None, problem: Union["Problem", None] = None, solver_fixed_factors: Union[dict, None] = None, problem_fixed_factors: Union[dict, None] = None, model_fixed_factors: Union[dict, None] = None, file_name_path: Union[str, None] = None):
        """There are two ways to create a ProblemSolver object:
            1. Provide the names of the solver and problem to look up in ``directory.py``.
            2. Provide the solver and problem objects to pair.
        """
    ...
    def run(self, n_macroreps: int):
        ...
        with Pool(processes=7) as process_pool:
            ...
            # Grab all the data out of the result
            for mrep in range(n_macroreps):
                self.all_recommended_xs[mrep], __, self.timings[mrep] = result.get()[mrep]
                self.all_intermediate_budgets[mrep] = list(range(self.iteration_num))
        print("Finished running {} macroreplications in {} seconds.".format(n_macroreps, round(time.time() - self.function_start, 3)))
    ...
    def run_multithread(self, mrep: int) -> tuple:
        ...
        # Trim the recommended solutions and intermediate budgets
        if len(recommended_solns) > self.iteration_num:
            recommended_solns = recommended_solns[:self.iteration_num]
        elif len(recommended_solns) < self.iteration_num:
            recommended_solns = recommended_solns + [recommended_solns[-1]] * (self.iteration_num - len(recommended_solns))
    ...
    def bootstrap_sample(self, bootstrap_rng: "MRG32k3a", normalize: bool = True) -> list["Curve"]:
        ...
        if self.crn_across_budget and not self.crn_across_macroreps:
            for idx in range(self.n_macroreps):
                ...
                # a given macroreplciation.
                bs_postrep_idxs = bootstrap_rng.choices(range(self.n_postreps), k=self.n_postreps)
                for budget in range(self.iteration_num):
                    # If solution is x0...
                    if self.all_recommended_xs[mrep][budget] == self.x0:
                        est_objectives.append(bs_initial_obj_val)
                    # ...else if solution is x*...
...
def trim_solver_results_it(iteration_num: list[int], problem: "Problem", recommended_solns: list["Solution"], intermediate_budgets: list[int]) -> tuple[list["Solution"], list[int]]:
    """Trim solutions recommended by solver after problem's max budget.
...
def plot_progress_curves_it(experiments: list["ProblemSolver"], plot_type: str, beta: float = 0.50, normalize: bool = True, all_in_one: bool = True, n_bootstraps: int = 100, conf_level: float = 0.95, plot_CIs: bool = True, print_max_hw: bool = True) -> list[str]:
    """Plot individual or aggregate progress curves for one or more solvers
    on a single problem with x-axis iteration numbers.
...
def setup_plot(plot_type: str, solver_name: str = "SOLVER SET", problem_name: str = "PROBLEM SET", normalize: bool = True, budget: Union[int, None] = None, beta: Union[float, None] = None, solve_tol: Union[float, None] = None):
    ...
    if plot_type == "all":
        ...
        title = f"CDF-Solvability Profile for {solver_name}\nProfile of CDFs of {round(solve_tol, 3)}-Solve Times"
    elif plot_type == "quantile_solvability":
