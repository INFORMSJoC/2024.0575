...
def mean_of_curves(curves):
    ...
    #mean_y_vals = [np.mean([curve.lookup(x_val) for curve in curves]) + 7 for x_val in unique_x_vals]
    mean_y_vals = [np.mean([curve.lookup(x_val) for curve in curves]) for x_val in unique_x_vals]
    # LOG_SCALE
    mean_y_vals = np.log10(mean_y_vals)
    mean_curve = Curve(x_vals=unique_x_vals.tolist(), y_vals=mean_y_vals)
    return mean_curve

def mean_of_curves_flat_1(curves):
    unique_x_vals = np.unique([x_val for curve in curves for x_val in curve.x_vals])
    mean_y_vals = np.ones(len(unique_x_vals))*1.6137832021131744
    mean_curve = Curve(x_vals=unique_x_vals.tolist(), y_vals=mean_y_vals)
    return mean_curve

def mean_of_curves_flat_2(curves):
    unique_x_vals = np.unique([x_val for curve in curves for x_val in curve.x_vals])
    mean_y_vals = np.ones(len(unique_x_vals))*1.4765
    mean_curve = Curve(x_vals=unique_x_vals.tolist(), y_vals=mean_y_vals)
    return mean_curve

def mean_of_curves_flat_3():
    unique_x_vals = list(range(0, 2001))
    mean_y_vals = np.ones(len(unique_x_vals))*0.8469885055804586
    mean_curve = Curve(x_vals=unique_x_vals, y_vals=mean_y_vals)
    return mean_curve

def mean_of_curves_flat_global():
    unique_x_vals = list(range(0, 2001))
    mean_y_vals = np.ones(len(unique_x_vals))*0
    mean_curve = Curve(x_vals=unique_x_vals, y_vals=mean_y_vals)
    return mean_curve
...
class Experiment(object):
    ...
    def run(self, n_macroreps):
        ...
        #self.all_intermediate_computational_burden = []
...
def plot_progress_curves(experiments, plot_type, beta=0.50, normalize=True, all_in_one=True, plot_CIs=True, print_max_hw=True):
    ...
    if all_in_one:
        ...
        plt.legend(handles=solver_curve_handles, labels=entire_labels, loc="upper right")
...
def setup_plot(plot_type, solver_name="SOLVER SET", problem_name="PROBLEM SET", normalize=True, budget=None, beta=None, solve_tol=None):
    ...
    if normalize:
        ...
        #log - 1
        #plt.ylabel("Optimality Gap", size=14)
        plt.xlim((0, budget))
