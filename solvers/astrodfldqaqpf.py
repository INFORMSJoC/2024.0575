...
class ASTRODFLDQAQPF(Solver):
    """ VMI-2STRO-3 on regularized objective function (figure 14)
    Attributes
    ----------
    name : string
        name of solver
    ...
    base.Solver
    """
    def __init__(self, name="ASTRODFLDQAQPF", fixed_factors=None):
        ...
        self.specifications = {
            ...
            "sampling_version": {
                "description": "1: VMI-2STRO-1 (lambda), 2: VMI-2STRO-2 (variance model), 3: VMI-2STRO-3 (hybrid)",
                "datatype": float,
                "default": 3
            },
            "penalty_function_constant": {
                "description": "constant for the penalty function",
                "datatype": float,
                "default": 1
            },
            "cv": {
    ...
    def construct_model(self, x_k, delta, k, problem, expended_budget, kappa, new_solution, visited_pts_list, num_implementation):
        ...
        sampling_version = self.factors["sampling_version"]
        pf_constant = self.factors["penalty_function_constant"]
        cv = self.factors["cv"]
        #print(cv)
        j = 0
        ind_success = 0
        ...
        while True:
            ...
            for i in range(2 * problem.dim + 1):
                ...
                if (k == 1) and (i == 0):
                    fval.append(-1 * problem.minmax[0] * new_solution.objectives_mean + pf_constant*new_solution.objectives_var)
                ...
                # if iteration k-1 is unsuccessful, we can reuse the replications for x_k
                    ...
                    if sample_size >= self.get_stopping_time(k, sig2, delta_k, kappa, problem.dim):
                        fval.append(-1 * problem.minmax[0] * new_solution.objectives_mean + pf_constant*sig2)
                        interpolation_solns.append(new_solution)
                    else:
                        needed_replications = min(self.get_stopping_time(k, sig2, delta_k, kappa, problem.dim) - sample_size, max(budget-expended_budget,2))
                        #print(sig2, self.get_stopping_time(k, sig2, delta_k, kappa, problem.dim) - sample_size, budget-expended_budget)
                ...
                # else if we reuse one design point, reuse the replications
                    ...
                    # Sampling
                    if sample_size >= self.get_stopping_time(k, sig2, delta_k, kappa, problem.dim):
                        fval.append(-1 * problem.minmax[0] * visited_pts_list[f_index].objectives_mean + pf_constant*sig2)
                        ...
                        num_implementation += 1
                        expended_budget += needed_replications + overhead_costs
                        fval.append(-1 * problem.minmax[0] * visited_pts_list[f_index].objectives_mean + pf_constant*visited_pts_list[f_index].objectives_var)
                        interpolation_solns.append(visited_pts_list[f_index])
                ...
                # for new points, we need to run the simulation
                    ...
                    if sampling_version == 3:
                        # pilot run 
                        if k > 1 and ind_success == 1:
                            ...
                            if estimated_var > sig2_centerpoint + cv*delta_k:
                                pilot_run = ceil(max(lambda_min, 2 * log(problem.dim,10)) * max(log(k + 0.1, 10) ** (1.01), 1))
                            problem.simulate(design_set_solution, pilot_run)
                            num_implementation += 1
                            expended_budget += pilot_run + overhead_costs
                            sample_size = pilot_run
                            sig2 = design_set_solution.objectives_var
                        ...
                        # Sampling
                        if sample_size >= self.get_stopping_time(k, sig2, delta_k, kappa, problem.dim):
                            fval.append(-1 * problem.minmax[0] * design_set_solution.objectives_mean + pf_constant*sig2)
                            interpolation_solns.append(design_set_solution)
                            #print('first')
                        else:
                            needed_replications = min(self.get_stopping_time(k, sig2, delta_k, kappa, problem.dim) - sample_size, max(budget-expended_budget,2))
                            problem.simulate(design_set_solution, needed_replications)
                            num_implementation += 1
                            expended_budget += needed_replications + overhead_costs
                            fval.append(-1 * problem.minmax[0] * design_set_solution.objectives_mean + pf_constant*design_set_solution.objectives_var)
                            interpolation_solns.append(design_set_solution)

                    elif sampling_version == 2:
                        # pilot run
                        if k > 1:
                            estimated_var = max(self.evaluate_model(np.array(Z[i]), q_r),0.00001)
                            pilot_run = min(self.get_stopping_time(k, estimated_var, delta_k, kappa, problem.dim), max(budget-expended_budget,2))
        ...
        return fval, Y, q, q_r, ind_success, grad, Hessian, delta_k, expended_budget, interpolation_solns, visited_pts_list, num_implementation
    ...
    def iterate(self, k, delta_k, delta_max, problem, visited_pts_list, new_x, expended_budget, budget_limit, recommended_solns, intermediate_budgets, kappa, new_solution, var_data, num_implementation):
        ...
        fval, Y, q, q_r, ind_success, grad, Hessian, delta_k, expended_budget, interpolation_solns, visited_pts_list, num_implementation = self.construct_model(new_x, delta_k, k, problem, expended_budget, kappa, new_solution, visited_pts_list, num_implementation)
        ...
        if norm(grad) != 0:
            ...
            fval_tilde = -1 * problem.minmax[0] * candidate_solution.objectives_mean + pf_constant * new_solution.objectives_var
