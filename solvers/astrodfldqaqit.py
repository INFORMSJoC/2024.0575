...
class ASTRODFLDQAQIT(Solver):
    """ VMI-2STRO-3 for figure 13
    Attributes
    ----------
    name : string
        name of solver
    ...
    base.Solver
    """
    def __init__(self, name="ASTRODFLDQAQIT", fixed_factors=None):
                ...
                # for new points, we need to run the simulation
                    ...
                    if sampling_version == 3:
                        ...
                        if k > 1 and ind_success == 1:
                            ...
                            if estimated_var > sig2_centerpoint + 10*delta_k:
    ...
    def iterate(self, k, delta_k, delta_max, problem, visited_pts_list, new_x, expended_budget, budget_limit, recommended_solns, intermediate_budgets, kappa, new_solution, var_data, num_implementation):
        ...
        if norm(grad) != 0:
            ...
            fval_tilde = -1 * problem.minmax[0] * candidate_solution.objectives_mean + new_solution.objectives_var
    ...
    def solve(self, problem):
        ...
        while k < 30:
            ...
            problem.simulate(record_solution, 200)
        ...
        if problem.name == 'MAXCUT-1':
            file_name_function = 'figures/function_maxcut_vmi.npy'

        # Save Delta history to plot it
        if os.path.exists(file_name_function) == True:
            all_delta = np.load(file_name_function)
            all_delta = np.vstack((all_delta, function_list))
            np.save(file_name_function,all_delta)
        else:
