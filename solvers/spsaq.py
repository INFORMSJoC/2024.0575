...
Summary
-------
Simultaneous perturbation stochastic approximation (SPSA) is an algorithm for optimizing quantum problems with multiple unknown parameters.
"""
import numpy as np
...
class SPSAQ(Solver):
    ...
    base.Solver
    """
    def __init__(self, name="SPSAQ", fixed_factors=None):
    ...
    def solve(self, problem):
        ...
        expended_budget = 0
        overhead_cost = self.factors["overhead_burden"]
        ...
        expended_budget = self.factors["n_reps"] + overhead_cost
        ...
        for _ in range(int(self.factors["n_loss"] / (2 * self.factors["gavg"]))):
            ...
            for _ in range(self.factors["gavg"]):
                ...
                expended_budget += 2 * self.factors["n_reps"] + 2 * overhead_cost
                # Estimate gradient.
