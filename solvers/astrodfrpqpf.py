...
class ASTRODFRPQPF(Solver):
    """ ASTRO-DF on regularized objective function (figure 14)
    Attributes
    ----------
    name : string
        name of solver
    ...
    base.Solver
    """
    def __init__(self, name="ASTRODFRPQPF", fixed_factors=None):
        ...
        while True:
            ...
            for i in range(2 * problem.dim + 1):
                ...
                # for new points, we need to run the simulation
                    ...
                    design_point_solution = self.create_new_solution(tuple(Y[i][0]), problem)
                    visited_pts_list.append(design_point_solution)
                    # pilot run # ??check if there is existing result
                    pilot_run = int(max(lambda_min, .5 * problem.dim) - 1)
                    problem.simulate(design_point_solution, pilot_run)
                    expended_budget += pilot_run + overhead_costs
                    sample_size = pilot_run

                    # adaptive sampling
                    while True:
                        problem.simulate(design_point_solution, 1)
                        expended_budget += 1 + overhead_costs
                        sample_size += 1
                        sig2 = design_point_solution.objectives_var
                        if sample_size >= self.get_stopping_time(k, sig2, delta_k, kappa, problem.dim) or expended_budget >= budget:
                            break
                    fval.append(-1 * problem.minmax[0] * design_point_solution.objectives_mean + pf_constant*sig2)
                    interpolation_solns.append(design_point_solution)
    ...
    def iterate(self, k, delta_k, delta_max, problem, visited_pts_list, new_x, expended_budget, budget_limit, recommended_solns, intermediate_budgets, kappa, new_solution):
        ...
        fval_tilde = -1 * problem.minmax[0] * candidate_solution.objectives_mean + pf_constant*sig2
